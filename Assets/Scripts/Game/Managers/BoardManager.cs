...
namespace SpiritWorld.Managers {
  public class BoardManager : MonoBehaviour, IObserver {
    ...
    public void notifyOf(IEvent @event) {
      switch (@event) {
        case TileSelectionManager.TileFeatureUpdated tsmTFU:
          updateFeature(tsmTFU.updatedTile, tsmTFU.updatedFeature);
          break;
        case TileSelectionManager.TileFeatureDestroyed tsmTFD:
          destroyFeature(tsmTFD.updatedTile, tsmTFD.removedLayer);
          break;
        case TileSelectionManager.TileFeatureDropsLeftover tsmTFDL:
          dropItemsOnTile(tsmTFDL.updatedTile, tsmTFDL.drops);
          break;
        default:
          break;
      }
    ...
    void dropItemsOnTile(Tile tile, Item[] drops) {
      if (inUseControllers.TryGetValue(tile.parentChunkKey, out GridController chunkController)) {
        chunkController.addItemDrops(tile, drops);
      }
    }
  }
}
