  ...
  public class ItemDropController : MonoBehaviour {
    ...
    void Update() {
      if (item != null) {
        ...
        /// spin the highlight
        if (itemHilight != null) {
          itemHilight.transform.rotation = Quaternion.Euler(
            ItemHilight.transform.rotation.x,
            ItemHilight.transform.rotation.y + rotationSpeed,
            ItemHilight.transform.rotation.z
          );
        }
      }
    }
    ...
    public void setItem(Item item) {
      ...
      if (item != null && item.type != null) {
        itemModel = Instantiate(ItemDataMaper.GetModelFor(item), transform);
    ...
    void OnCollisionEnter(Collision collision) {
      if (item != null && item.quantity > 0 && collision.gameObject.CompareTag("Player")) {
        tryToGetPickedUpBy(collision.gameObject.GetComponent<LocalPlayerMovementController>());
      }
    }
